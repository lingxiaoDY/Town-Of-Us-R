...
namespace TownOfUs
{
    ...
    public class OpenDoorConsoleCanUse
    {
            ...
            ref bool __state)
        {
            ...
            if ((playerControl.Is(RoleEnum.幻影) && !Role.GetRole<Phantom>(playerControl).Caught) || (playerControl.Is(RoleEnum.冤魂) && !Role.GetRole<Haunter>(playerControl).Caught) && playerInfo.IsDead)
            {
